class VendingMachine:
    ...
    def vend(self):
        ...
        difference = self.funds - self.price
        if difference < 0:
            return 'You must add ${0} more funds.'.format(-difference)
        if difference > 0:
            ms = 'Here is your {0} and ${1} change.'.format(self.product, difference)
        else:
            ms = 'Here is your {0}.'.format(self.product)
        self.stock -= 1
        self.funds = 0
        return ms
...
class Coin:
    ...
    def worth(self):
        ...
        extra_cent = 0
        if coin_age > beyond:
            extra_cent = coin_age - beyond
        return self.cents + extra_cent 
...
def store_digits(n):
    ...
    def helper(result, n):
        digit, n = n % 10, n // 10
        if not n:
            return Link(digit, result) 
        else:
            return helper(Link(digit, result), n)
    return helper(Link.empty, n)
