...
def couple(s, t):
    ...
    for index in range(len(s)):
        coupled.append([s[index], t[index]])
    return coupled
...
def closer_city(lat, lon, city_a, city_b):
    ...
    gap = distance(target, city_a) - distance(target, city_b) 
    return get_name(city_a) if gap < 0 else get_name(city_b)
...
def check_city_abstraction():
    ...
    abstraction's underlying implementation is known as violating the abstraction barrier,
    and we never want to do this!
    The nature of the abstraction barrier guarantees that changing the implementation of
    an ADT shouldn't affect the functionality of any programs that use that ADT, as long as
    the constructors and selectors were used properly.
...
# Similar with rational structure in Programming Language(A) section4
    ...
    if (label(t) == 'berry'):
        ...
        for b in branches(t):
            if berry_finder(b) == True:
                return True
            # print("result of b: ", berry_finder(b))
    return False
