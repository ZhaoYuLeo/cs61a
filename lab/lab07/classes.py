...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for pc in player.deck.cards:
            pc.attack += other_card.attack
            pc.defense += other_card.defense
        new_deck_cards = []
        for oc in opponent.deck.cards:
            if oc.attack != other_card.attack or oc.defense != other_card.defense:
                new_deck_cards.append(oc)
        opponent.deck.cards = new_deck_cards
