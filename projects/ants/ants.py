...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting:
            ...
            random = rANTdom_else_none(self.place.bees)
            if random: 
                self.eat_bee(random)
        # END Problem 6
...
class LaserAnt(ThrowerAnt):
    ...
    damage = 2
    weakened_each_distance = 0.2
    weakened_each_shot = 0.05
    implemented = True   # Change to True to view in the GUI
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        damage = (self.damage
                 - (self.weakened_each_shot * self.insects_shot)
                 - (self.weakened_each_distance * distance))
        if damage < 0:
            damage = 0
        return damage
        # END Problem Optional 5
